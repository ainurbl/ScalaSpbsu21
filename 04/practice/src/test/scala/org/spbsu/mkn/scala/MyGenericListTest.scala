...
import org.spbsu.mkn.scala.MyGenericList.{IntComp, fromSeq, size, sort, sum}
...
class MyGenericListTest extends AnyFunSuite {
  ...
  test("sort") {
    ...
    assert(sort(fromSeq(Seq(2, 1, 3))) == fromSeq(Seq(1, 2, 3)))
    assert(sort(fromSeq(Seq(2, 3, 1))) == fromSeq(Seq(1, 2, 3)))
    assert(sort(fromSeq(Seq(3, 1, 2))) == fromSeq(Seq(1, 2, 3)))
